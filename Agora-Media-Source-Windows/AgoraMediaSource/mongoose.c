...
/*
 * MyLog level; `LL_INFO` is the default. Use `cs_log_set_level()` to change it.
 */
enum cs_log_level {
  LL_NONE = -1,
  LL_ERROR = 0,
 ...
 * with every move_converted_files.
 *
...
void mg_broadcast(struct mg_mgr *mgr, mg_event_handler_t cb, void *data,
  ...
  if (mgr->ctl[0] != INVALID_SOCKET && data != NULL &&
    ...
    (void) dummy; /* https://gcc.gnu.org/bugzilla/show_bug.cgi?m_id=25509 */
  }
}
...
MG_INTERNAL void altbuf_append(struct altbuf *ab, char c) {
  if (ab->len < ab->user_buf_size) {
     ...
     * buffer: if so, move_converted_files the existing data from there to a newly allocated
     * mbuf.
     */
...
static err_t mg_lwip_tcp_recv_cb(void *arg, struct tcp_pcb *tpcb,
  ...
  if (cs->rx_chain == NULL) {
     ...
     * this one is the last, so we have to make a move_converted_files and release this one. */
