...
void CAGEngineEventHandler::onLeaveChannel(const RtcStats& stat)
{
	if (m_hMainWnd != NULL)
	{
		::PostMessage(m_hMainWnd, WM_LEAVECHANNEL_SUCCESS, 0, 0);
	}
}
...
void CAGEngineEventHandler::onNetworkQuality(uid_t uid, int txQuality, int rxQuality)
{
	...
	if (m_hMainWnd != NULL)
		::PostMessage(m_hMainWnd, WM_NETWORK_QUALITY, (WPARAM)lpData, 0);
}
...
{
	LPConnectionStatusChanged lpData = new ConnectionStatusChanged;
	lpData->state = state;
	lpData->reason = reason;
	if (m_hMainWnd != NULL)
		::PostMessage(m_hMainWnd, WM_CONNECTION_STATUS_CHANGED, (WPARAM)lpData, 0);
}
