...
void CEnterChannelDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDCHUSERNAME_CHANNEL, m_userID);
...
//	MoveWindow(0, 0, 320, 450, 1);
	...
	m_userID.MoveWindow(80, 180, 320, 22, TRUE);
	m_userID.SetFont(&m_ftHead);
	m_userID.SetCaretPos(CPoint(12, 148));
	m_userID.ShowCaret();
	m_userID.SetTip(__T("UserID"));



	//m_ctrEncKey.MoveWindow(80, 230, 160, 22, TRUE);
  ...
  //  m_ctrEncKey.SetTip(LANG_STR("IDS_CHN_KEYTIP"));
	m_ctrEncKey.ShowWindow(SW_HIDE);
    ...
    m_cmbEncType.MoveWindow(290, 280, 120, 22, TRUE);
	...
	m_cmbEncType.ShowWindow(SW_HIDE);


	m_cmbUserType.Create(WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | CBS_OWNERDRAWVARIABLE, CRect(ClientRect.Width() / 2 + 1, 168, 180, 32), this, IDC_CMBENCTYPE_CHANNEL);
	m_cmbUserType.MoveWindow(180, 240, 120, 22, TRUE);
	m_cmbUserType.SetFont(&m_ftHead);

	m_cmbUserType.SetButtonImage(IDB_CMBBTN, 12, 12, RGB(0xFF, 0x00, 0xFF));
	m_cmbUserType.SetFaceColor(RGB(0xFF, 0xFF, 0xFF), RGB(0xFF, 0xFF, 0xFF));
	m_cmbUserType.InsertString(0, __T("Host"));
	m_cmbUserType.InsertString(1, __T("Client"));
...
bool CEnterChannelDlg::IsHost()
{
	if (m_cmbUserType.GetCurSel() == 0)
		return true;
	else
		return false;
...
CString CEnterChannelDlg::GetUserID()
{
	CString strChannelName;

	m_userID.GetWindowText(strChannelName);
