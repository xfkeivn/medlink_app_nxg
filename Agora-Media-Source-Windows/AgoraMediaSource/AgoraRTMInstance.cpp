...
long long int CAgoraRTMInstance::SUBSCRIBE_PEERS_ONLINE_STATUS_REQUEST = 123456LL;
...
void CAgoraRTMInstance::InitRTMService() {
  ...
  m_callManager = m_rtmService->getRtmCallManager(m_RtmCallback);
  if (ret) {
    exit(0);
  }
}
...
m_rtmService(nullptr), m_invitation(nullptr){
	callBack->setRTMInstance(this);
  InitRTMService();
}
...
bool CAgoraRTMInstance::subscribePeersOnlineStatus(const char* peerIds[], int peerCount)
{
	if (m_rtmService != nullptr) {
		int nret = m_rtmService->subscribePeersOnlineStatus(peerIds, peerCount, SUBSCRIBE_PEERS_ONLINE_STATUS_REQUEST);
		return true;
	}
...
void CAgoraRTMInstance::SendInvitationPeer(UINT uid, std::string channelName, std::string callFrom)
{
	m_invitation = m_callManager->createLocalCallInvitation(int2str(uid).c_str());
	m_invitation->setChannelId(channelName.c_str()); 
	m_invitation->setContent(callFrom.c_str());
	invitations.push_back(m_invitation);
	m_callManager->sendLocalInvitation(m_invitation);	
}

void CAgoraRTMInstance::CancelInvitation(string user_id)
{
	if (invitations.size() > 0)
	{
		for (int i = 0; i < invitations.size(); i++)
		{
			if (invitations[i]->getCalleeId() == user_id)
			{
				logInfo("Cancel the specific invitation of " + user_id);
				int state = invitations[i]->getState();
				logInfo("The state of the outgoing call invitation:" + to_string(state));
				int result = m_callManager->cancelLocalInvitation(invitations[i]);
				logInfo("The result of cancelling call invitation:" + to_string(result));
				if (state == LOCAL_INVITATION_STATE_RECEIVED_BY_REMOTE || result != 0)
				{
					logInfo("Send CancelRemoteInvitationCMD");
					CAgoraObject::GetAgoraObject()->SendCancelRemoteInvitationCMD(user_id);
				}
				vector<ILocalCallInvitation*>::iterator iter = std::find(invitations.begin(), invitations.end(), invitations[i]);
				if (iter != invitations.end())
				{
					iter = invitations.erase(iter);
				}
				break;
			}
		}
...
void CAgoraRTMInstance::CancelInvitations()
{
	//if (m_invitation != NULL)
	//	m_callManager->cancelLocalInvitation(m_invitation);
	//m_callManager->release();
	if (invitations.size() > 0)
	{
		logInfo("=============CancelInvitations============");
		for (int i = 0; i < invitations.size(); i++)
		{
			string user_id = invitations[i]->getCalleeId();
			logInfo("Cancel the invitation to " + user_id);
			int m = m_callManager->cancelLocalInvitation(invitations[i]);
			vector<ILocalCallInvitation*>::iterator iter = std::find(invitations.begin(), invitations.end(), invitations[i]);
			if (iter != invitations.end())
			{
				iter = invitations.erase(iter);
...
void CAgoraRTMInstance::AcceptInvitation(IRemoteCallInvitation *invitation)
{
	m_callManager->acceptRemoteInvitation(invitation);
}

void CAgoraRTMInstance::RefuseInvitation(IRemoteCallInvitation *invitation)
{
	m_callManager->refuseRemoteInvitation(invitation);
	//m_callManager->release();

}

void CAgoraRTMInstance::onLocalInvitationAccept(ILocalCallInvitation *localInvitation)
{
	vector<ILocalCallInvitation*>::iterator iter = std::find(invitations.begin(), invitations.end(), localInvitation);
	if (iter != invitations.end())
	{
		iter = invitations.erase(iter);
	}
...
void CAgoraRTMInstance::onLocalInvitationRefused(ILocalCallInvitation *localInvitation)
{
	vector<ILocalCallInvitation*>::iterator iter = std::find(invitations.begin(), invitations.end(), localInvitation);
	if (iter != invitations.end())
	{
