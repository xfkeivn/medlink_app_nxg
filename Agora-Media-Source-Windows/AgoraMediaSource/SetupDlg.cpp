...
BOOL CSetupDlg::OnInitDialog()
{
	...
	if (CAGConfig::GetInstance()->IsAutoSaveEnabled())
...
void CSetupDlg::InitCtrls()
{
    ...
    if (CAGConfig::GetInstance()->IsAutoSaveEnabled())
        nResolutionIndex = CAGConfig::GetInstance()->GetSolution();
    else
        nResolutionIndex = 15;
...
void CSetupDlg::OnBnClickedBtnconfirmSetup()
{
	...
	if (m_ckSaveSettings.GetCheck() == TRUE) {
		CAGConfig::GetInstance()->EnableAutoSave(TRUE);
		CAGConfig::GetInstance()->SetSolution(m_cbxVideoProfile.GetCurSel());
	}
	else
		CAGConfig::GetInstance()->EnableAutoSave(FALSE);
}
