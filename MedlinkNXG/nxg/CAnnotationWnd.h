...
class CAnnotationCwnd : public CDialogEx
{
	DECLARE_DYNAMIC(CAnnotationCwnd)

private:
	CAGVideoWndNXG* m_parent = nullptr;
public:
	CAnnotationCwnd();
	virtual ~CAnnotationCwnd();
	void setParent(CAGVideoWndNXG* parent);
// 对话框数据
...
public:
	...
	void setDrawingStatus(DRAWING_STATUS status);
	void KeepNoTransparent(CDC *pDC);

	void processRemoteAnnotationEvent(DrawingEvent messageEvent);
...
public:
	...
	//FOR TESTING,
	DRAWING_STATUS m_drawing_status = NO_DRAWING;

	IDrawingPanelObserver *m_drawing_observer = nullptr;
	//VideoSourceMgr *m_video_source_mgr = nullptr;
