...
CLanguageSet *CLanguageSet::GetInstance()
{
	...
	int pos = CString(CLanguageSet::m_szSelfPath).Find(APP_NAME);
	CString newstr = CString(CLanguageSet::m_szSelfPath).Left(pos);
	_tcscpy_s(CLanguageSet::m_szSelfPath, MAX_PATH, newstr + _T("Language\\"));
